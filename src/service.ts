...
export async function getCrateDetails(crateName: string): Promise<{
  ...
  try {
    ...
    throw new Error(
      `failed to get crate details for ${crateName}: ${
...
export async function getCrateDocumentation(
  ...
  try {
    logger.info(
      `getting documentation for crate: ${crateName}${
    ...
    throw new Error(
      `failed to get documentation for crate ${crateName}: ${
        (error as Error).message
      }`
    );
  }
...
export async function getSourceCode(
  ...
  if (!path?.trim()) {
    throw new Error(
      "Path is required and cannot be empty. Use format like 'lib.rs' or 'src/main.rs'"
    );
  }
  ...
  try {
    ...
    if (!response) {
      throw new Error(
        `Could not find source code at any of the attempted URLs: ${possibleUrls.join(
          ", "
        )}`
      );
    }
